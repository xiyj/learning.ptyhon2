...
def list_project_buckets(project_name) :
    if False :
        import requests
        # response = requests.get('https://www.googleapis.com/storage/v1/b?project=%s&key={YOUR_API_KEY}')
        url = 'https://www.googleapis.com/storage/v1/b?project=%s'%(project_name)
        print("access url : %s"%url)
        response = requests.get('https://www.googleapis.com/storage/v1/b?project=%s'%(project_name))
        print(response.json())
    storage_client = storage.Client()
...
if __name__ == '__main__':
    ...
    # list_project_buckets('freeswtich-gsr')
    list_blobs('freeswitch-gsr')
    ...
    if False :
        print("make blob public")
        make_blob_public('freeswitch-gsr', 'out.wav')
